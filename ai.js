...
function Ai(populationSize, maxDepth, simulationRepetition) {
  this.snake;
  this.population = [];
  this.offsprings = [];
  ...
  this.runEvolution = function(n) {
    for(let i=0; i<n-1; i++) {
      ...
      console.log("run -> " + i + " best -> " + Math.round(this.population[0].fitness/this.simulationRepetition));
    ...
    this.runSimulation();
    this.population.sort(this.sortPopulation);
    console.log("last runs best -> " + Math.round(this.population[0].fitness/this.simulationRepetition));
  }

  //next time write more play less // choose trees based on fitness and change functions(dangerUp etc) for controllign the snkae
  this.Evolve = function() {

    let range = Math.round(this.populationSize/4);
    ...
    for(let i=0; i<this.populationSize; i++) {
      ...
      let a,b,c;
      a = this.randomBranch(randomTree1,1);
  ...
  this.runSimulation = function() {
    ...
    for(let j=0; j<this.populationSize; j++) {
      for(let i=0; i<this.simulationRepetition; i++) {
      // console.log("hi");
        ...
        while(!this.snake.checkCrash()) {
          ...
          //change?? test is when doing performence testing
          if( (move >= 500 && this.population[j].fitness < 2) || move > 50000) {
